...
このアプリは、**お使いのマシンで動かして使うもの**として作っています。
サーバーへ設置して第三者へ提供する運用はお控えください。

理由はネットワーク RTK（NTRIP）にあります。ブラウザから配信局へ直接つなげないため
サーバーが中継する必要があり、その経路には**配信局アカウントの認証情報が乗ります**。
公開してしまうと、他人の認証情報を預かる立場に立つことになります。
また配信局から見れば利用者全員のアクセスがサーバー 1 台の IP に集まるため、
配信局側の判断で遮断されると、利用者全員が同時に使えなくなります。

そのため中継機能は、**ローカルから開いたときだけ**動くようにしてあります
（[判定の仕組み](#ネットワーク-rtk-はローカル実行時のみ)）。
サーバーへ置いても中継は動作せず、これを外す設定も用意していません。
作者が公開している版も同じビルドで、中継機能は動きません（CLAS 補正のみ利用できます）。

ライセンスは [MIT License](LICENSE) なので、改変も再配布も自由です。
それを制限するものではありませんが、上記の事情から公開運用はお控えいただくようお願いします。
改変版を公開される場合は、本家と紛らわしい名称の使用もお控えください。
...
これを支えているのは、待ち受けアドレスをループバックに固定していることです。
`npm run dev` と `npm run start` はそれぞれ `next dev -H 127.0.0.1` /
`next start -H 127.0.0.1` を実行し、同じマシンからの接続しか受け付けません。
他所からの接続は OS が TCP の時点で断つため、リクエストの中身では動かせません。

そのうえで API 側でも、`Host` がループバック（`localhost`、127.0.0.0/8、`::1`）を
指すことを確かめています。こちらは待ち受けを広げて起動された場合
（`-H 0.0.0.0` を渡す、前段にリバースプロキシを置く、など）への備えです。
ただし `Host` はブラウザ以外のクライアントから詐称できるので、
**待ち受けを広げた時点でこの保証は失われます。** ローカル実行の前提を崩さないでください。

設定は要りません。`npm run dev` でも `npm run start` でも、`localhost` で開けばそのまま使えます。
それ以外の場所から開かれた場合は API が 404 を返し、画面には補正ソースとして
ネットワーク RTK を選んだときに案内が出ます。

**これを外す環境変数は用意していません。** 中継を公開状態で動かす使い方は想定していないためです
（[公開サーバーでの運用について](#公開サーバーでの運用について)）。
...
接続先を特定の配信局だけに絞りたい場合は、環境変数 `NTRIP_ALLOWED_HOSTS` に
カンマ区切りで列挙します。
...
ライセンス上の制限は設けていませんが、[公開サーバーでの運用について](#公開サーバーでの運用について)
に記載した事情がありますので、あわせてお読みください。
