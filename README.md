...
2. **2つの補正測位モード対応**
   ...
   - **ネットワークRTK (NTRIP) モード**: NTRIP Caster (rtk2go.com, 各種キャスター等) から補正データ (RTCM) を取得し、受信機へ注入してRTK測位を実現。
     - この機能はサーバー側で配信局へ中継するため、**ローカルで起動したときのみ有効**です（[理由と判定方法](#ネットワーク-rtk-はローカル実行時のみ)）。CLAS モードは受信機だけで完結するため、この制限を受けません。
...
Node.js の動かない共用サーバーでも、静的ファイルとして書き出せば FTP で上げるだけで公開できます。
このアプリは受信機との通信も電文の解析も地図もすべてブラウザ内で完結するため、
ネットワーク RTK 以外の機能はサーバーが無くてもそのまま動きます。

### 前提

- **HTTPS が使えること。** Web Serial API は保護されたコンテキストでしか動きません。
  HTTP のままでは受信機に接続できず、アプリとして成立しません
- Apache の `mod_headers` が使えること（`.htaccess` でセキュリティヘッダを付けるため）

### 1. ビルド設定を書く

リポジトリの直下に `.env.production.local` を作ります。このファイルは Git の管理外で、
`npm run dev` では読まれないため、ローカルでの開発には影響しません。
...
- `BASE_PATH`: 置き場所がサブディレクトリのときだけ指定します（`https://example.com/rtk/` なら `/rtk`）。
  ドメイン直下へ置くなら行ごと削除してください。**指定を誤ると CSS と JS がすべて 404 になります**
- `SITE_ORIGIN`: OGP 画像の絶対 URL を組み立てる基準です。`BASE_PATH` は含めません
...
`out/` に一式が出ます。`.htaccess` もこの中へ入るので、別途用意する必要はありません。

### 3. アップロードする

`out/` の**中身**を、公開ディレクトリの目的の場所へ FTP / SFTP で転送します。

```
out/ の中身  →  /public_html/rtk/
```

`out` フォルダごと置くと `https://example.com/rtk/out/` になってしまうので、中身だけを移します。
`.htaccess` は隠しファイルなので、FTP クライアントの設定で**隠しファイルを表示する**ようにしてください。

### 4. 確認する

`https://example.com/rtk/` を Chrome で開き、次を確かめます。

- URL が `https://` になっている（Web Serial に必要）
- 「接続」ボタンが押せる（`非対応` と出る場合は HTTPS か、Chromium 系ブラウザかを確認）
- 補正ソースで「ネットワークRTK」を選ぶと、ローカル実行を案内するパネルが出る

### この構成での制限

- **ネットワーク RTK は使えません。** 中継 API が成果物に含まれないためで、これは意図した動作です
  （[理由](#ネットワーク-rtk-はローカル実行時のみ)）。UI にもその旨が出ます
- CLAS（みちびき L6）による補正は、そのまま使えます
...
| `NTRIP_ENABLED` | （未設定＝ローカルからのアクセス時のみ有効） | `true` にすると、ループバック以外から開いた場合もネットワーク RTK を有効にします |
| `STATIC_EXPORT` | （未設定＝通常のビルド） | `true` にすると静的ファイルとして書き出します（`out/`）。API ルートは成果物へ含まれません |
| `BASE_PATH` | （未設定＝ドメイン直下） | サブディレクトリへ置く場合の接頭辞（例: `/rtk`） |
...
ネットワーク RTK（NTRIP）は、このアプリがサーバー側から第三者の配信局へ TCP 接続する
唯一の機能です。公開されたサーバーで動かすと、利用者の配信局アカウントの認証情報が
その中継サーバーを通り、配信局から見ればすべてのアクセスが 1 つの IP に集まります。
さらにストリームを流し続けられる時間は PaaS ごとに異なり、多くの環境では数分で切れます。

そのため、この機能は**ローカルで開いたときだけ**有効になります。判定は 2 段構えです。

1. 既知のマネージド環境の環境変数（`VERCEL` / `RENDER` / `NETLIFY` など）が見えたら拒否
2. そのうえで `Host` がループバック（`localhost`、127.0.0.0/8、`::1`）を指すことを要求

ローカルで動かす場合、設定は要りません。`npm run dev` でも `npm run start` でも、
`localhost` で開けばそのまま使えます。公開環境では API が 404 を返し、画面には
補正ソースとしてネットワーク RTK を選んだときに案内が出ます。

`Host` はブラウザ以外のクライアントから詐称できるため、1 段目を併せて置いています。
一覧に無いプラットフォームは `Host` の判定だけで守られるので、公開時は
下記の `NTRIP_ALLOWED_HOSTS` も併せて設定してください。

VPS などで意図して中継サーバーを立てる場合は、明示的に開けられます。

```bash
NTRIP_ENABLED=true npm run start
```

なおこの変数は、画面側の出し分けについてはビルド時に確定します
（サーバーコンポーネントで読むため）。ビルド時にも同じ値を渡してください。
