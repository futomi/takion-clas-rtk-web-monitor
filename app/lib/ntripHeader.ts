 ...
 * サーバーに肥大したリクエストを第三者の Caster へ送らせられる（増幅の踏み台）。
 * 実際に載る量は前段の上限（ストリームはリクエストボディ、Source-table はリクエスト行の
 * 長さ）でも頭打ちになるが、上限は経路ごとに違ううえ、ここは組み立ての最後の関門なので
 * それとは独立に抑えておく。実在する値はいずれもこの上限より遥かに短い。
 */
